...
public class CvdsBackApplicationTests {
	...
	// Test para la clase principal CvdsBackApplication
	@Test
	void testMainApplicationStartup() {
		// Este test verifica que la clase principal puede ejecutarse sin excepciones
		// Llamamos al método main con argumentos nulos para simular el inicio de la aplicación
		assertDoesNotThrow(() -> {
	...
	void testSpringApplicationRun() {
		// Test usando PowerMockito para verificar que SpringApplication.run es llamado
		// con los parámetros correctos
		try (MockedStatic<SpringApplication> mockSpringApplication = mockStatic(SpringApplication.class)) {
			CvdsBackApplication.main(new String[]{"arg1", "arg2"});

			mockSpringApplication.verify(() ->
							SpringApplication.run(CvdsBackApplication.class, new String[]{"arg1", "arg2"}),
					times(1)
			);
		}
	}
	...
	@Test
	void testSpringBootAnnotations() {
		// Verifica que las anotaciones correctas están presentes en la clase principal
		Class<?> appClass = CvdsBackApplication.class;

		assertTrue(appClass.isAnnotationPresent(SpringBootApplication.class),
				"La clase CvdsBackApplication debe tener la anotación @SpringBootApplication");

		assertTrue(appClass.isAnnotationPresent(EnableMongoRepositories.class),
				"La clase CvdsBackApplication debe tener la anotación @EnableMongoRepositories");
	}
